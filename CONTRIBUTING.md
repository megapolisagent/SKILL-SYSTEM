...
## Сравнить локальную копию агента с библиотекой

Не телеметрия и не постоянная связь между репозиториями — Skill System не видит агентов сама (§8 `SKILL-SYSTEM.md`). Разовая ручная проверка по явно указанному пути, когда нужно узнать, не разошлась ли копия у конкретного агента с тем, что лежит здесь:

```bash
# Один Skill — путь к файлу SKILL.md или к папке Skill'а
python3 scripts/skillctl.py compare <имя> --path "/путь/к/агенту/skills/<имя>"

# Все Skills сразу — папка skills/ конкретного агента
python3 scripts/skillctl.py compare-installed --dir "/путь/к/агенту/skills"
```

Сравнивает по содержимому (hash), не по дате. Пропускает молча то, чего нет в библиотеке (может быть Skill, специфичный только этому агенту) и то, что не установлено локально.

Три возможных ответа, не два:

- **совпадает** — буквально идентичен канону.
- **канон + локальная адаптация** — каноническая часть (всё до заголовка `## Локальная адаптация`) совпадает буквально; после этого заголовка — свой, не сверяемый слой (например: «делай X, а результат записывай сюда»). Это ожидаемо, не расхождение.
- **⚠ РАСХОДИТСЯ** — каноническая часть отличается сама по себе, не только добавленной адаптацией. Это настоящее расхождение: либо забытый синхрон, либо у агента ушла вперёд сама методика (тогда решение — зафиксировать явно, как это было сделано для `repository-design`, а не молчать).

Маркер `## Локальная адаптация` — фиксированная строка, распознаётся только как отдельная строка (не как часть другого текста), должен быть последним разделом файла.
