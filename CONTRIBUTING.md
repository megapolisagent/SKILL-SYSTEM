...
Сравнивает по содержимому (hash), не по дате. Пропускает молча то, чего нет в библиотеке (может быть Skill, специфичный только этому агенту) и то, что не установлено локально. Расхождение — не ошибка сама по себе: может быть намеренным (Skill у агента ушёл вперёд канона, решение зафиксировано отдельно) или забытым синхроном — какое из двух, решает человек, не команда.
