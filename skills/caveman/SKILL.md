...
> Перенесено 2026-09-04 (владелец → ENGINEER) из `JuliusBrussee/caveman` (GitHub) — сама папка `skills/` источника явно MIT (`LICENSING.md`: «Existing Caveman skill stays MIT and untouched»), инженерный движок/проксисервер (BSL-1.1) не переносится и не используется — только текст этого скилла. Репозиторий содержит ещё ~20 сопутствующих скиллов (cavecrew, caveman-compress, caveman-stats и др.) — не перенесены по отдельности, доступны в источнике при реальной потребности.
