...
> Перенесено 2026-08-27 (владелец → ENGINEER) из `coreyhaines31/marketingskills` (GitHub, MIT license) как справочный материал для AI Marketing Strategist — он видит область CRM/данных (замечает нестыковки, направляет к специалисту), но не исполняет её сам (`AI Marketing Strategist/HOME.md`, «Понимаю и советую, не исполняю»). Не подключено как чей-либо активный Skill — лежит в общей библиотеке до появления узкого специалиста, которому это нужно исполнять. Оригинальный текст ниже не переведён и не адаптирован под дом — сохранён как первоисточник. Кросс-ссылки на `cold-email`/`emails`/`pricing` в описании выше — это skills из репозитория-источника; `pricing` в этом доме тоже перенесён (см. `skills/pricing/`), остальных нет.
