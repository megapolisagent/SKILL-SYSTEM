...
> Перенесено 2026-09-04 (владелец → ENGINEER) из `heygen-com/hyperframes` (GitHub, реальный источник — README методички ссылался на пустую обёртку `hyperframes/hyperframes`, найдена и проверена по перекрёстной ссылке из самого файла). Импортирован основной `skills/hyperframes/SKILL.md`. Репозиторий содержит ~19 сопутствующих специализированных скиллов (hyperframes-cli, hyperframes-animation, faceless-explainer, product-launch-video, slideshow, talking-head-recut и др.) — не перенесены по отдельности, доступны при появлении видео-агента.
